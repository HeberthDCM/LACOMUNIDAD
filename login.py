...
def generarMenu(usuario):
    ...
    with st.sidebar: # Creamos una barra lateral para el menú
        ...
        if rol in ['ventas','admin','comercial']:
            st.page_link("pages/0300_Reportes.py", label="Ventas", icon=":material/finance:")    
        if rol in ['compras','admin','comercial']:
            st.page_link("pages/0200_NuevosNacidos.py", label="Nuevos Nacidos", icon=":material/child_friendly:")
        if rol in ['personal','admin','compras']:
            st.page_link("pages/0400_.py", label="Personal", icon=":material/group:") 
        if rol in ['contabilidad','admin']:
            st.page_link("pages/0500_.py", label="Contabilidad", icon=":material/payments:") 
        if rol in ['contabilidad','admin']:
            st.page_link("pages/0100_Asistencia.py", label="Asistencia2", icon=":material/assignment_ind:")  
    ...
    with st.sidebar: # Menú lateral
        ...
        if st.secrets["ocultarOpciones"]=="True": # Verificamos el valor del secreto "ocultarOpciones"
            ...
            for index, row in dfPaginas.iterrows():
                if index==1:
                    st.subheader("Nuevos nacidos") # Subtítulo para los tableros
                if index==6:
                    st.subheader("REDES")
                if index==7:
